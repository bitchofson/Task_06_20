...
public class SimpleHashMap<K, V> implements Serializable {
    ...
    // Начальная мощность по умолчанию, должна быть равна степени двойки.
    private static final int DEFAULT_INITIAL_CAPACITY = 16;

    // Максимальная должна быть равна 2, максимальная мощность = 1073741824.
    private static final int MAXIMUM_CAPACITY = 1 << 30;

    // Коэффициент нагрузки, используемый, если он не указан в конструкторе.
    private static final float DEFAULT_LOAD_FACTOR = 0.75f;

    // Таблица, измененная по мере необходимости. Длина должна быть равна степени двойки.
    private Entry<K, V>[] entryTable;

    // Количество пар ключ-значение.
    private int size;

    // Следующее значение размера, при котором необходимо изменить размер (мощность * коэффициент нагрузки).
    private int threshold;

    // Коэффициент нагрузки для хэш-таблицы.
    ...
    // Значение хэша по ключу
    final int hash(Object key) {
        return Math.abs(key.hashCode()) % entryTable.length;
    }

    // Возвращает таблицу после предварительного удаления устаревших записей.
    ...
    public void putAll(SimpleHashMap<? extends K, ? extends V> m) {
    ...
    // Переносит все записи из таблицы src в таблицу dest
    ...
    // Код специального случая для containsValue с нулевым аргументом.
